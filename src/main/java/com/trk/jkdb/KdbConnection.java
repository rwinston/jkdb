...
public class KdbConnection{
  ...
  /**
   Initializes a new {@link KdbConnection} instance.
...
//    j=0;
  ...
  LocalDate rd(){
    ...
    return Instant.ofEpochMilli(i==ni?nj:gl(i*86400000L+k)).atZone(zone).toLocalDate();
  }
  void w(LocalDate d){
    long j=d.toEpochDay();//*86400000;
    w(j==nj?ni:(int)(lg(j*86400000)/86400000-10957));
  }
  Time rt(){
    int i=ri();
  ...
  Object r() throws UnsupportedEncodingException{
    ...
    switch(t){
      ...
      case 11:
        ...
        return S;
      case 12:
        LocalDateTime[] P=new LocalDateTime[n];
        for(;i<n;i++)
          P[i]=rp();
        return P;
      case 13:
